...
def send_sctp_packets(destination_ip, destination_port, interval_sec, num_packets, packet_size):
    startTime = time.time()
    sctp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_SCTP)
    sctp_socket.settimeout(5)
    ...
    try:
        ...
        return
    sctp_socket.settimeout(1)  # Set socket timeout to 1 second
    ...
    for i in range(num_packets):
        ...
        try:
            response = sctp_socket.recv(512*3000)
            received_packets += 1
            end_time = time.time()
...
exp_times = 3
num_packets = 500
filename = 'sctp_{}_result.csv'.format(args.topology_name)
    ...
    for msg_size in msg_sizes:
        for interval in intervals:
            for exp_number in range(1, exp_times + 1):
                result = send_sctp_packets(args.destination_ip, args.destination_port, interval, num_packets, msg_size)
                if result:
